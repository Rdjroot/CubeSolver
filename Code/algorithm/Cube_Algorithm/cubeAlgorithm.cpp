...
//int twistMoveTable[NTWIST][Nmove];		// all角块原始方向 --18种转动-->  结果's方向编码
//int flipMoveTable[NFLIP][Nmove];		// 棱块方向变换表
//int sliceMoveTable[NSLICE][Nmove];		// 中层棱块组合排列变换表
//
//int cpMoveTable[NCP][Nmove];			// 角块位置变换表
//int eudpMoveTable[NEUDP][Nmove];		// 上下层棱块变换表
//int empMoveTable[NEMP][Nmove];			// 中层棱块变换表
...
void initPruneTable1(vector<int>& pruneTable, vector<vector<int>>& moveTable)
{
	...
	/*cout << "cur table size is: " << sz << " , the biggest depth is: " << depth << endl;*/
}
...
void initTables()
{
	...
	outFile << endl;
	for (int i = 0; i < empMoveTable.size(); i++)
	{
		outFile << i << ": ";
		for (int j = 0; j < 18; j++)
		{
			outFile << empMoveTable[i][j] << ", ";

		}
		outFile << endl;
	}
	outFile << "code  " << "step  " << endl;
	for (int i = 0; i < EMpPruneTable.size(); i++)
	{
		outFile << i << "	 " << EMpPruneTable[i] << endl;
	}
...
void search1(int twist, int flip, int slice, int curDepth)
{
	...
	//oss << "search1( " << twist << ", " << flip << ", " << slice << ", " << curDepth << ");";
	if (curDepth == 0)
	{
		// 阶段一任务完成，已达成状态
...
//// 编码和解码测试
//void testCoding(CubieCube& a)
//{
//	// 测试方向解码和编码
//	int a_cornero = encodeCornerO(a);
//	CubieCube bco = decodeCornerO(a_cornero);
//	cout << "1  " << std::boolalpha << a.equalCornerO(bco) << endl;
//
//	int a_edgeo = encodeEdgeO(a);
//	CubieCube beo = decodeEdgeO(a_edgeo);
//	cout << "2  " << std::boolalpha << a.equalEdgeO(beo) << endl;
//
//	// 测试角块位置还原
//	int a_cornerp = encodeCornerP(a);
//	CubieCube acp = decodeCornerP(a_cornerp);
//	cout << "3  " << std::boolalpha << a.equalCornerP(acp) << endl;
//
//	// 测试棱块位置康托展开编码
//	int a_edgeundp = encodeEdgeUnDP(a);
//	CubieCube e_aeudp = decodeEdgeUnDP(a_edgeundp);
//	int e_edgeundp = encodeEdgeUnDP(e_aeudp);
//	cout << "4   compare: " << std::boolalpha << (a_edgeundp == e_edgeundp) << endl;
//
//	int a_edgemp = encodeEdgeMP(a);
//	CubieCube e_aemp = decodeEdgeMP(a_edgemp);
//	int e_edgemp = encodeEdgeMP(e_aemp);
//	cout << "5   compare: " << std::boolalpha << (e_edgemp == a_edgemp) << endl;
//
//	int a_edgemp2 = encodeEdgeMP2(a);
//	CubieCube e_aemp2 = decodeEdgeMP2(a_edgemp2);
//	int e_edgemp2 = encodeEdgeMP2(e_aemp2);
//	cout << "6   compare: " << std::boolalpha << (e_edgemp2 == a_edgemp2) << endl;
//
//}
...
int main()
{
	...
	vector<string> latex = { "U2", "L2", "B2", "U", "L2", "D'", "B2", "U2", "L2", "F2",
							 "L2", "U'", "B", "U", "R'", "D2", "L", "D", "U2" };
	// 初始化魔方
	for (auto lt : latex)
	{
		int t = moveMap[lt];
